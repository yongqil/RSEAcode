...
public class MOEADBuilder implements AlgorithmBuilder<AbstractMOEAD<DoubleSolution>> {
	public enum Variant {MOEAD, ConstraintMOEAD, MOEADDRA, MOEADSTM, MOEADD, MOEADMy, MOEADDU, cMOEADMy, cMOEAD,MOEAD_RS, RSEA} ;
	...
	public MOEADBuilder setPlot(boolean plot) {
		this.plot = plot;
		return this;
	}

	public MOEADBuilder setNormalize(boolean normalnized) {
	...
	public AbstractMOEAD<DoubleSolution> build() {
		...
		if (moeadVariant.equals(Variant.MOEAD)) {
			algorithm = new MOEAD(problem, populationSize, resultPopulationSize, maxEvaluations, mutation,
					...
					maximumNumberOfReplacedSolutions, neighborSize,normalnized);
		}else if (moeadVariant.equals(Variant.RSEA)) {
			algorithm = new RSEA(problem, populationSize, resultPopulationSize, maxEvaluations, mutation,
					crossover, functionType, dataDirectory, neighborhoodSelectionProbability,
					maximumNumberOfReplacedSolutions, neighborSize,plot,normalnized);
		}else if (moeadVariant.equals(Variant.MOEAD_RS)) {
		algorithm = new MOEAD_RS(problem, populationSize, resultPopulationSize, maxEvaluations, mutation,
				crossover, functionType, dataDirectory, neighborhoodSelectionProbability,
				maximumNumberOfReplacedSolutions, neighborSize,plot,normalnized);
	}
		return algorithm ;
