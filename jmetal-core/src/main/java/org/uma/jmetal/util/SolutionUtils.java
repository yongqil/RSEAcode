...
public class SolutionUtils {
  ...
  public static <S extends Solution<?>> double angleBeBetweenObjectives(S firstSolution, S secondSolution, double[] nadirPoint, double[] idealPoint, boolean normalized) {
	  ...
	  for (int i = 0; i < firstSolution.getNumberOfObjectives(); i++) {
		if(normalized){
			...
			if(nadirPoint[i]-idealPoint[i] > 10e-10){
				f1 = (firstSolution.getObjective(i)-idealPoint[i])/(nadirPoint[i]-idealPoint[i]);
				f2 = (secondSolution.getObjective(i)-idealPoint[i])/(nadirPoint[i]-idealPoint[i]);
			}else{
				f1 = (firstSolution.getObjective(i)-idealPoint[i])/10e-10;
				f2 = (secondSolution.getObjective(i)-idealPoint[i])/10e-10; 
			}
			a += f1*f2;
			b += f1*f1;
			c += f2*f2;
		}else{
			a += firstSolution.getObjective(i)*secondSolution.getObjective(i);
			b += firstSolution.getObjective(i)*firstSolution.getObjective(i);
			c += secondSolution.getObjective(i)*secondSolution.getObjective(i);
		}
	  }
