...
public class SolutionListUtils {
  ...
  /**
   * Returns a matrix with the angle between each pair of solutions in the population.
   * Distances are measured in the objective space
   * @param solutionSet
   * @return
   */
  public static  <S extends Solution<?>> double [][] angleMatrix(List<S> solutionSet, double[] nadirPoint, double[] idealPoint, boolean normalized) {
     double [][] distance = new double [solutionSet.size()][solutionSet.size()];
     for (int i = 0; i < solutionSet.size(); i++){
       distance[i][i] = 0.0;
       for (int j = i + 1; j < solutionSet.size(); j++){
         distance[i][j] = SolutionUtils.angleBeBetweenObjectives(solutionSet.get(i),solutionSet.get(j), nadirPoint, idealPoint, normalized);                
         distance[j][i] = distance[i][j];            
       }
     }
     return distance;
