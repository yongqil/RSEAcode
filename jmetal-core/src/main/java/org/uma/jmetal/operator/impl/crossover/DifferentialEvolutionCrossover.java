      ...
      for (int j = 0; j < numberOfVariables; j++) {
        if (crRandomGenerator.getRandomValue(0.0, 1.0) < cr || j == jrand) {
          ...
          if (value < child.getLowerBound(j)) {
        	  if( crRandomGenerator.getRandomValue(0.0, 1.0)<0.5){
        		  value =  child.getLowerBound(j);
        	  }else{
        		  value =  (child.getLowerBound(j)+currentSolution.getVariableValue(j))/2;
        	  }
          }
          if (value > child.getUpperBound(j)) {
        	  if(crRandomGenerator.getRandomValue(0.0, 1.0)<0.5){
        		  ...
        		  value =  (child.getUpperBound(j)+currentSolution.getVariableValue(j))/2;
        	  }
          }
